...
def app(image_dir, threads, model):
    ...
    for i in range(runTotal):
        if i % 500 == True:
            print("Processed ", i, "/", runTotal, "images")
    ...
    ''' post-processing '''
    config = json.load(open(glob("*.json")[-1]))
    classes = sorted(config["class_labels"], key=str.lower)
    ...
    for i in range(len(out_q)):
        ...
        ground_truth = images[image_dir.split("/")[-1] + "/" + listimage[i]]
...
def main():
    ...
    ap.add_argument('-m', '--model', type=str, default='model_dir/densenetx.xmodel',
                    help='Path of xmodel. Default is model_dir/densenetx.xmodel')
